...
//Therefore, we only need to consider factors up to ¡Ìn because, if n is divisible by some number p, then n = p ¡Á q and since p ¡Ü q, we could derive that p ¡Ü ¡Ìn.
//
...
//Then we look at the next number, 3. Similarly, all multiples of 3 such as 3 ¡Á 2 = 6, 3 ¡Á 3 = 9, ... must not be primes, 
...
//Now, all multiples of 5 such as 5 ¡Á 2 = 10, 5 ¡Á 3 = 15, 5 ¡Á 4 = 20, 5 ¡Á 5 = 25, ... can be marked off. 
...
//because 5 ¡Á 2 = 10 was already marked off by multiple of 2, similarly 5 ¡Á 3 = 15 was already marked off by multiple of 3. 
...
/*  Works, but not efficient
    public int countPrimes(int n) {
        List<Integer> lstPrime = new ArrayList<Integer>();
        if (n <= 2) return 0;
        
        lstPrime.add(2);
        
        for (int i=3; i<n; i++) {
        	boolean bPrime = IsPrime(i, lstPrime);
        	if (bPrime) lstPrime.add(i);
        }
        
        return lstPrime.size();
    }
	
    
    public boolean IsPrime(int n, List<Integer> lstPrime) {
    	int nSqrt = (int)Math.sqrt(n);
    	for (Integer nPrime:lstPrime) {
    		if (nPrime > nSqrt) break;
    		if (n % nPrime == 0) return false;
    	}
