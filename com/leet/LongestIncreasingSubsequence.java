...
public class LongestIncreasingSubsequence {
	...
	public int lengthOfLIS(int[] nums) {
	    ...
	    int len = nums.length;
	    int[] seqEnd = new int[len+1]; //index is ranking, value is raw index of nums[]. 
	                                   //seqEnd[i] remembers the raw index (in nums[]) of ith (starting from 1) element (ith element = ranking ith)
	    seqEnd[1] = 0;
	    int lisLen = 1;
	...
	public int findPos(int[] nums, int[] seqEnd, int lisLen, int index){
	    int start = 1;  //Ranking
	    int end = lisLen;  //Current highest ranking
	    ...
	    return start;  //The ranking for nums[index] within nums[0~index]
	}
