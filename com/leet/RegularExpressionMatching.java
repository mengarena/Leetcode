...
public class RegularExpressionMatching {
    ...
    public boolean isMatch(String s, String p) {
        ...
        boolean dp[][] = new boolean[lenS+1][lenP+1];   //Default false, Whether s[0~i] p[0~j] matches, 
	                                                //i-1 in s corresponds i in dp
        int i, j;
        ...
        for (j=2; j<=lenP; j++) {
        	if (p.charAt(j-1) == '*' && dp[0][j-2] == true) dp[0][j] = true;   //dp[0][j-2] is by p.charAt(j-3); 
		                                                                   //when p.charAt(j-1) = '*', 
		                                                                   //it can cancel p.charAt(j-2)
        }
        ...
        for (i=1; i<=lenS; i++) {
        	...
        	if (p.charAt(j-1) == '.' || p.charAt(j-1) == s.charAt(i-1)) dp[i][j] = dp[i-1][j-1];
        		...
        		dp[i][j] = dp[i-1][j] || dp[i][j-2];    
			              //dp[i][j-2]: not using "a*" part,   
        			      //dp[i-1][j]:  "a*" part has been used/matched until s[i-2] 
			              //                 (there is one index gap between dp and s/p)
        			      //             "a*" part always could give a 'a" to match s[i-1], 
			              //                  so if s[i-2] and p[j-1] matches, 
        			      //              s[i-1] and p[j-1] also matches
        	    } else {
        		dp[i][j] = dp[i][j-2];   //in P:   mbcda* ,  "a*" part not used 
			                         //(i.e. '*' is consumed with 'a', because "a" does not match with s[i-1]
        	    }
        	} 
            }
        }
