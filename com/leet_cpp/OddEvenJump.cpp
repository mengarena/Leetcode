...
public:
    int oddEvenJumps(vector<int>& A) {
        int n = A.size();
        vector<int> odd_next(n, -1);  // The destination of odd jump from current position
        vector<int> even_next(n, -1); // The destination of even jump from current position
        map<int, int> num_idx;        // A[i], i (ascending sorted)
        
        // Find the odd/even jump destination from each position
        for (int i=n-1; i>=0; --i) {
        ...
        vector<bool> odd(n, false), even(n, false);  // Whether odd/even jump from current postion could reach end
        odd[n-1] = even[n-1] = true;
        int res = 1;  // The end position itself is a good starting index
        
        for (int i=n-2; i >= 0; --i) {
            if (odd_next[i] != -1) {
                odd[i] = even[odd_next[i]];  //Whether the odd jump from position i could reach end depends on 
                                             //whether the even jump from the jump destination of position i could reach end 
            }
            ...
            if (odd[i]) res++;     //Since starting just is always odd, so if an odd jump from i could reach end, this position i is one good starting index
        }
