...
public:
    ...
    string getFileInfo(string s, unordered_map<string, string>& name_content) {
        istringstream ss(s);
        string path;
        ss >> path;
        string fileinfo;
        ...
        while (ss >> fileinfo) {
            size_t pos1 = fileinfo.find("(");
            string filename = fileinfo.substr(0, pos1);
            string filecontent = fileinfo.substr(pos1+1, fileinfo.length()-pos1-2);
            name_content[filename] = filecontent;
        }
    ...
    vector<vector<string>> findDuplicate(vector<string>& paths) {
        unordered_map<string, unordered_set<string>> m;  // file content (or hash of file content),  path+filename
        
        for (auto s:paths) {
            unordered_map<string, string> ncs;
            string path = getFileInfo(s, ncs);
            
            for (auto nc:ncs) {
                m[nc.second].insert(path + "/" + nc.first);
            }
        }
        ...
        for (auto mm:m) {
            if (mm.second.size() > 1) {
                ans.push_back(vector<string>(mm.second.begin(), mm.second.end()));
            }
        }
