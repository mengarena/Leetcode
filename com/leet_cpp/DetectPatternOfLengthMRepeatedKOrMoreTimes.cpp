...
class Solution {
public:
    bool isPattern(vector<int>& arr, int startIdx, vector<int>& pattern, int cnt, int m) {
        for (int i=0; i<cnt; ++i) {
            int rawIdx = startIdx + m*i;
            for (int j=0; j<m; ++j) {
                if (arr[rawIdx+j] != pattern[j]) {
                    return false;
                }
            }
        }
    ...
    bool containsPattern(vector<int>& arr, int m, int k) {
        ...
        for (int i=0; i<=n-m*k; ++i) {
            ...
            if (isPattern(arr, i+m, pattern, k-1, m)) {
                return true;
            }
        }
