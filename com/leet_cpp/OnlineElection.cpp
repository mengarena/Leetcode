...
public:
    ...
    // Means:  persons: [0,1,1,0,0,1,0];   times: [0,5,10,15,20,25,30]
    //         t for q():  [3],[12],[25],[15],[24],[8]
    TopVotedCandidate(vector<int>& persons, vector<int>& times) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (counts[persons[i]] >= maxCount) {
                cand = persons[i];
                maxCount = counts[persons[i]];
            }
            vv.push_back(make_pair(times[i], cand));
        }
    }
    ...
    int q(int t) {
        ...
        while (i <= j) {
            int mid = i + (j-i)/2;
            if (vv[mid].first == t) return vv[mid].second;
            if (vv[mid].first > t) {
                j = mid-1;
            } else {
                candId = vv[mid].second;
                i = mid+1;
            }
        }
...
private:
    vector<pair<int, int>> vv;   // Time, top id
};
