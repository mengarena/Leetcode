...
public:
    ...
    char slowestKey(vector<int>& releaseTimes, string keysPressed) {
        unordered_map<char, int> keydurs;
        int n = keysPressed.length();
        int longestDur = 0;
        for (int i=0; i<n; ++i) {
            if (i == 0) {
                keydurs[keysPressed[i]] = releaseTimes[i];
            } else {
                keydurs[keysPressed[i]] = max(keydurs[keysPressed[i]], releaseTimes[i] - releaseTimes[i-1]);
            }
            
            longestDur = max(keydurs[keysPressed[i]], longestDur);
        }
        
        char ret = 'a';
        for (char i='z'; i>='a'; --i) {
            if (keydurs[i] == longestDur) {
                ret = i;
                break;
            }
        }
