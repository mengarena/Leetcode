...
public:
    ...
    // 78%
    // Strategy:  Stack to record the index
    // Each time get the node, check whether it could be the "next greater" of any previous nodes
    vector<int> nextLargerNodes(ListNode* head) {
        ...
        while (head) {
            ...
            while (!stkIdx.empty() && ans[stkIdx.top()] < val) {
                ans[stkIdx.top()] = val;
                stkIdx.pop();
            }
